...
CMD="${PERL_PATH} ${COMMAND_CNACS}/subscript_exome/cat_depth.pl "${files[@]}" | \
...
# Mean of depth
CMD="cut -f 5 ${ORGDIR}/stats/depth_summary.txt | grep -v NA | perl -nle 'print $_ if ( $_ < 10 )' > ${ORGDIR}/stats/depth_mean.txt"
echo ${CMD}
eval ${CMD}
...
CMD="cut -f 6 ${ORGDIR}/stats/depth_summary.txt | grep -v NA | perl -nle 'print $_ if ( $_ < 1 )' > ${ORGDIR}/stats/depth_coefvar.txt"
echo ${CMD}
eval ${CMD}
