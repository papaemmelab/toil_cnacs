...
CMD="${PERL_PATH} ${COMMAND_CNACS}/subscript_exome/bait_gc_ref.pl ${ORGDIR}/sequence.fa ${MAX_FRAG_LENGTH} ${LENG} ${SAMPLE_SEX} ${PAR_BED} ${ORGDIR}/${ID}/tmp/pos2gc_leng${LENG_NUM} ${ORGDIR}/${ID}/tmp/gc2num_leng${LENG_NUM}.txt"
echo ${CMD}
eval ${CMD}

# load %GC for each fragment
# calculate GC-stratified coverage
CMD="${PERL_PATH} ${COMMAND_CNACS}/subscript_exome/stratify_gc_ref.pl ${ORGDIR}/${ID}/tmp/mapped_leng${LENG_NUM}.bed ${ORGDIR}/${ID}/tmp/pos2gc_leng${LENG_NUM} ${ORGDIR}/${ID}/tmp/gc2num_leng${LENG_NUM}.txt > ${ORGDIR}/${ID}/tmp/gc2rate_leng${LENG_NUM}.txt"
echo ${CMD}
eval ${CMD}
check_error $?
