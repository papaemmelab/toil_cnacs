...
CMD="${PERL_PATH} ${COMMAND_CNACS}/subscript_target/bait_gc.pl ${BAIT_FA} ${MAX_FRAG_LENGTH} ${LENG} ${ORGDIR}/${ID}/tmp/pos2gc_leng${LENG_NUM}.txt ${ORGDIR}/${ID}/tmp/gc2num_leng${LENG_NUM}.txt"
echo ${CMD}
eval ${CMD}

# load %GC for each fragment
# calculate GC-stratified coverage
CMD="${PERL_PATH} ${COMMAND_CNACS}/subscript_target/stratify_gc.pl ${ORGDIR}/${ID}/tmp/mapped_leng${LENG_NUM}.bed ${ORGDIR}/${ID}/tmp/pos2gc_leng${LENG_NUM}.txt ${ORGDIR}/${ID}/tmp/gc2num_leng${LENG_NUM}.txt > ${ORGDIR}/${ID}/tmp/gc2rate_leng${LENG_NUM}.txt"
echo ${CMD}
eval ${CMD}
check_error $?
