...
while (<SIGNAL>) {
	...
	if ( defined $start2end{$pos} ) {
		...
		if ( $calling > 0 ) {
			...
			my @gene_list;
			my @start_info = split(/\t/, $cna_start);
			my $cna_chr = 'chr' . $start_info[0];
			$cna_chr = 'chrX' if ( $start_info[0] == 23 );
			$cna_chr = 'chrY' if ( $start_info[0] == 24 );
			foreach my $cur_gene ( sort keys %gene2chr ) {
				next if ( $gene2chr{$cur_gene} ne $cna_chr );
				next if ( $gene2end{$cur_gene} < $start_info[1] );
				next if ( $gene2start{$cur_gene} > $cna_end );
				push(@gene_list, $cur_gene);
			}
			$pos2info{$cna_start} = '"' . $id . '"' . "\t" . $cna_start . "\t" . $cna_end . "\t" . $calling . "\t" . $depth_output . "\t" . $as_output . "\t" . join(",", @gene_list);
